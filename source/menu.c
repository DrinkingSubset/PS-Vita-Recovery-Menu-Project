...
static Menu *build_cpu_menu(void);
...
static MenuItem g_cpu_items[8];
static Menu     g_cpu_menu;
...
static void build_cpu_menu_impl(Menu *parent) {
    int n = 0;
    int count = cpu_get_preset_count();
    for (int i = 0; i < count && n < 7; i++) {
        g_cpu_items[n].label   = cpu_get_preset_name(i);
        g_cpu_items[n].type    = ITEM_ACTION;
        g_cpu_items[n].enabled = 0;
        // We use a closure trick via a static index array
        // For simplicity, we handle CPU selection in menu_handle_input
        g_cpu_items[n].action  = NULL;
        g_cpu_items[n].submenu = NULL;
        n++;
    }
    g_cpu_items[n].label   = "< Back";
    g_cpu_items[n].type    = ITEM_BACK;
    g_cpu_items[n].action  = NULL;
    g_cpu_items[n].submenu = NULL;
    n++;

    g_cpu_menu.title    = "CPU Speed";
    g_cpu_menu.items    = g_cpu_items;
    g_cpu_menu.count    = n;
    g_cpu_menu.selected = cpu_get_current_preset();
    g_cpu_menu.parent   = parent;
}
...
static void build_advanced_menu_impl(Menu *main) {
    build_cpu_menu_impl(&g_advanced_menu);
    build_registry_menu_impl(&g_advanced_menu);

    int n = 0;
    g_advanced_items[n++] = (MenuItem){ "CPU Speed",       ITEM_SUBMENU,   0, NULL,              &g_cpu_menu      };
...
void menu_handle_input(Menu **current, SceCtrlData *old) {
    ...
    if (pressed(&ctrl, old, SCE_CTRL_CROSS)) {
            ...
            case ITEM_SUBMENU:
                if (item->submenu) {
                    ...
                    *current = item->submenu;
                    // Rebuild if needed
                    if (item->submenu == &g_cpu_menu)
                        build_cpu_menu_impl(m);
                }
                break;
        ...
        // CPU preset selection
        if (m == &g_cpu_menu && item->type == ITEM_ACTION) {
            cpu_apply_preset(m->selected);
        }
    }
